var view = { //Представление (визуализация)
    ...
    ships: [{ locations: [0, 0, 0], hits:['', '', '']},
            ...
            fire: function(guess) {
                for (var i = 0; i < this.numShips; i++) {
                    ...
                    if (ship.hits[index] === "hit") { //повторное попадание по кораблю
...
function parseGuess(guess) {
    ...
    if (guess === null || guess.length !== 2) {
        ...
        if (isNaN(row) || isNaN(column)) { //выявление строк и столбцов, которые не являются цифрами
            alert('Бро, это вне игры, совсем не туда шмальнул');
        } else if (row < 0 || row >= model.boardSize || column < 0 || column >= model.boardSize) { //Проверка диапозона от 0 до 6, запрос из model и преобразование строки в число
    ...
    processGuess: function(guess) {
        ...
        if (location) {
            ...
            if (hit && model.shipsSunk === model.numShips) {
                view.displayMessage('Поздравляю с победой! Тебе потребовалось ' + this.guesses + ' выстрелов и точность составила: ' + 9/this.guesses); //Добавил точность, при изменении количества кораблей, поменять 9
            }
        }
    }
};
...
function handleKeyPress(e) {
    var fireButton = document.getElementById('fireButton');
    if (e.keyCode === 13 || e.keyCode === 32) { //KeyCode 13 это нажатие клавиши enter 32 пробед
        fireButton.click();
        return false;
    }
}
